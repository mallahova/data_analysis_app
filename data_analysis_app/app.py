...
if st.session_state.data_file:
    ...
    if 'preprocessor' not in st.session_state:
        ...
        data = st.session_state.preprocessor.get_data()  # Use the preprocessed data from session state
    ...
    if st.session_state.current_page == 'preprocessing':
        ...
        if handle_nulls:
            method = st.sidebar.radio("Select method to handle nulls:", ("mean", "drop", "fill_with_0"))
            preprocessing_options["handle_nulls"] = {"method": method}
        if rename_columns:
            rename_map = {}
            old_col = st.sidebar.selectbox("Select column to rename", facade.data.columns)
            new_col = st.sidebar.text_input("Rename column to:")
            if old_col and new_col:
                rename_map[old_col] = new_col
            preprocessing_options["rename_map"] = rename_map
        if drop_columns:
            cols_to_drop = st.sidebar.multiselect("Select columns to drop", facade.data.columns)
        ...
        if preprocess_button:
            st.session_state.preprocessed_data = facade.preprocess_data(**preprocessing_options)
        ...
        if plot_type:
            if plot_type != 'heatmap' and plot_type != 'dimensionality_reduction':
                ...
                if plot_type in ["line", "scatter"]:
                    ...
                    if plot_type == "line":
                        additional_params["line_color"] = st.sidebar.color_picker("Line Color", "#0000FF")
                        additional_params["line_width"] = st.sidebar.slider("Line Width", 1, 10, 2)
                    ...
                    additional_params["bins"] = st.sidebar.slider("Number of Bins", 5, 100, 30)
                    additional_params["bar_color"] = st.sidebar.color_picker("Bar Color", "#00FF00")
            ...
            try:
                ...
                file_format = st.sidebar.selectbox("Choose file format", ["png", "jpeg", "svg"])
